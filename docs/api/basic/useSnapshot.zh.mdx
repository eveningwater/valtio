...
通常，Valtio 的快照（通过 <a href="/docs/api/advanced/snapshot">`snapshot()`</a> 创建）会在代理或其任何子代理发生*任何*更改时重新创建。
...
1. 由于 `useSnapshot` 使用代理在 `useSnapshot` 返回*之后*记录访问的方式，`useDebugValue` 中列出的字段在技术上来自*前一次*渲染。
